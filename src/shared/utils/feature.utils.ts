import type { ThemeBase } from '@typing';
...
export const setTheme = (theme: ThemeBase) => {
  ...
  localStorage.setItem(StorageKey.SetTheme, `${theme.id}`);
  localStorage.setItem(StorageKey.SetBackground, `${theme.name}-bg`);
};
...
export const saveVolumeToLocal = (val: number) => {
  localStorage.setItem(StorageKey.SetVolume, String(val));
  localStorage.setItem(StorageKey.SetCacheVolume, String(val !== 0 ? val : DEFAULT_VOLUME));
};
